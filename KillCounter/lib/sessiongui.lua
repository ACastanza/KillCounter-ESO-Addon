   ...
   for i=1,tkw.MaxLines do
	 ...
	 local function SortBy(sortField)
	    return function()
		  ...
		  table.sort(kills_table.DataLines, function(a,b) return a[sortField] < b[sortField] end)
	       end
	       KC_G.UpdateSessionKillsTable()
	    end
	 end
	 ...
	 if i == 1 then
	    ...
	    if j == 4 then 
	       ...
	       tkw.Lines[i].Columns[j].SortButton:SetAnchor(TOPLEFT,tkw.Lines[i].Columns[j],TOPLEFT,0,0)
	       tkw.Lines[i].Columns[j].SortButton.Desc = false
	       tkw.Lines[i].Columns[j].SortButton:SetHandler("OnClicked",SortBy(KilledBy))
	       tkw.Lines[i].Columns[j].SortButton:SetHidden(false)
	    end
	    if j == 5 then 
...
function KC_G.UpdateSessionKillsTable(...)
   ...
   for i = 2,tlw.MaxLines do
      ...
      curLine.Columns[5]:SetText(zo_strformat("<<1>>", GetClassName(GENDER_MALE, curData[Class])))
