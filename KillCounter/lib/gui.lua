...
function KC_G.settingsWindowGUISetup()
   ...
   for i,p in pairs(settingsArray) do
	 ...
	 tlw.Lines[i]:SetAnchor(TOPLEFT,tlw.Lines[i-1],BOTTOMLEFT,0,3)
      ...
      tlw.Lines[i].Columns[1] = WINDOW_MANAGER:CreateControl(nil,tlw.Lines[i],CT_LABEL)
      ...
      tlw.Lines[i].Columns[2] = WINDOW_MANAGER:CreateControl(nil,tlw.Lines[i],CT_BUTTON)
      tlw.Lines[i].Columns[2].Label = WINDOW_MANAGER:CreateControl(nil,tlw.Lines[i].Columns[2],CT_LABEL)
      tlw.Lines[i].Columns[2].Label:SetFont("ZoFontGameBold")
      tlw.Lines[i].Columns[2]:SetDimensions(tlw.Lines[i]:GetWidth()/2,30)
      tlw.Lines[i].Columns[2].Label:SetDimensions(tlw.Lines[i]:GetWidth()/2,30)

      tlw.Lines[i].Columns[2]:SetAnchor(TOPRIGHT,tlw.Lines[i],TOPRIGHT,85,8)
      ...
      tlw.Lines[i].Columns[2]:SetHandler("OnClicked",
					 function(self,delta)
					    local ret = KC_Fn.ToggleSetting(p[2])
					    settings_table.Lines[i].Columns[2].Label:SetText(ret and "On" or "Off") 
...
function KC_G.statsWindowGUISetup()
   ...
   for i=1,tkw.MaxLines do
	 ...
	 local function SortBy(sortField)
	    return function ()
		  ...
		  table.sort(kills_table.DataLines,function(a,b) return b[sortField] < a[sortField] end)
	       else --sort ascending 
		  table.sort(kills_table.DataLines,function(a,b) return b[sortField] > a[sortField] end)
	       end
	       KC_G.UpdateKillsTable()
   ...
   for i=1,tdw.MaxLines do
	 ...
	 local function SortBy(sortField)
	    return function(self,delta)
		  ...
		  table.sort(deaths_table.DataLines, function(a,b) return b[sortField] < a[sortField] end)
	       else
		  table.sort(deaths_table.DataLines, function(a,b) return b[sortField] > a[sortField] end)
	       end
	       KC_G.UpdateDeathsTable()
   ...
   tbgw.Sorcerer.Label = WINDOW_MANAGER:CreateControl(nil,tbgw.Sorcerer,CT_LABEL)
	 ...
	 if i == 1 then
	    if j == 1 then 
	       ...
	       tkbsw.Lines[i].Columns[j].SortButton:SetHandler("OnClicked",function(self,delta)
								  ...
								  if tkbsw.Lines[i].Columns[j].SortButton.Desc then
								     table.sort(killing_blow_spells_table.DataLines, function(a,b) return b.SpellName < a.SpellName end)
								  else
								     table.sort(killing_blow_spells_table.DataLines, function(a,b) return a.SpellName < b.SpellName end)
								  end
							       KC_G.updateKillingSpellTable()
							      end)
	    ...
	    if j == 2 then 
	       ...
	       tkbsw.Lines[i].Columns[j].SortButton:SetHandler("OnClicked",function(self,delta)
								  tkbsw.Lines[i].Columns[j].SortButton.Desc = not tkbsw.Lines[i].Columns[j].SortButton.Desc
								  if tkbsw.Lines[i].Columns[j].SortButton.Desc then
								     table.sort(killing_blow_spells_table.DataLines, function(a,b) return b.Kills < a.Kills end)
								  else
								     table.sort(killing_blow_spells_table.DataLines, function(a,b) return a.Kills < b.Kills end)
								  end
								  KC_G.updateKillingSpellTable() 
							      end)
	       tkbsw.Lines[i].Columns[j].SortButton:SetHidden(false)
...
function KC_G.Percentages(sourcelist, field)
   ...
   for ii,v in pairs(sourcelist) do
      ...
      if list[v[Class]] then
	 list[v[Class]] =  list[v[Class]] + v[field]
      end
...
function KC_G.UpdateKillsTable(...)
   ...
   for i = 2,tlw.MaxLines do
      ...
      curLine.Columns[6]:SetText(zo_strformat("<<1>>",GetClassName(GENDER_MALE,curData[Class])))
	 ...
	 for j=1,breakdown_table.MaxColumns do 
	    if i==1 then
		  ...
		  if j==2 then 
		     ...
		     for _,v in pairs(KC_G.savedVars.players) do
			if v[Kills] > 0 then
			   uniqueKilledPlayers = uniqueKilledPlayers + 1
			   end
		     end
		     breakdown_table.Lines[i].Columns[j]:SetText(uniqueKilledPlayers)
		  end
		  ...
		  if j==2 then 
			...
			if v[Alliance] == ALLIANCE_EBONHEART_PACT then 
			   ep = ep + v[Kills]
			elseif v[Alliance] == ALLIANCE_DAGGERFALL_COVENANT then 
			   dc = dc + v[Kills]
	       ...
	       if i==5 then
		  if j==2 then 
		     local temp, sorc, dk, nb, ward, necro = 0, 0, 0, 0, 0, 0
		     for ii,v in pairs(KC_G.savedVars.players) do
         ...
         elseif v[Class] == KC_CLASS_NECROMANCER then
		     ...
		     local max = math.max(temp, sorc, dk, nb, ward, necro)
			...
			breakdown_table.Lines[i].Columns[j]:SetText(zo_strformat("<<1>>", GetClassName(GENDER_MALE, KC_CLASS_SORCERER)) .. " (" .. max .. " Kills)")
		     elseif dk == max then
			breakdown_table.Lines[i].Columns[j]:SetText(zo_strformat("<<1>>", GetClassName(GENDER_MALE, KC_CLASS_DRAGONKNIGHT)) .. " (" .. max .. " Kills)")
		     elseif nb == max then
         ...
         breakdown_table.Lines[i].Columns[j]:SetText(zo_strformat("<<1>>", GetClassName(GENDER_MALE, KC_CLASS_WARDEN)) .. " (" .. max .. " Kills)")
		  ...
		  if j == 2 then
		     ...
		     for _,v in pairs(KC_G.savedVars.players) do
			if v[KilledBy] > 0 then
			   uniqueKilledByPlayers = uniqueKilledByPlayers + 1
			end
		     end
		     breakdown_table.Lines[i].Columns[j]:SetText(uniqueKilledByPlayers)
		  end
	       end
	       if i==9 then
		  if j==2 then 
      ...
      breakdown_death_graph.Sorcerer.Label:SetText(string.format("|Cababab%s: %.2f%% %d Deaths|r",zo_strformat("<<1>>", GetClassName(GENDER_MALE,KC_CLASS_SORCERER)),
      ...
      breakdown_death_graph.NightBlade.Label:SetText(string.format("|Cababab%s: %.2f%% %d Deaths|r",zo_strformat("<<1>>", GetClassName(GENDER_MALE,KC_CLASS_NIGHTBLADE)),
      ...
      --Templar Label
      breakdown_death_graph.Templar.Label:SetText(string.format("|Cababab%s: %.2f%% %d Deaths|r",zo_strformat("<<1>>", GetClassName(GENDER_MALE,KC_CLASS_TEMPLAR)),
...
function KC_G.ShowPlayer(player)
   ...
   if class ~= "" then
      player_view_window.ClassLabel:SetText("Class: " .. zo_strformat("<<1>>",GetClassName(GENDER_MALE, class)))
   else
