...
function KC_G.OnKillingBlow( eventCode , result , isError , abilityName, abilityGraphic, abilityActionSlotType, sourceName, sourceType, targetName, targetType, hitValue, powerType, damageType, log )
   ...
   --If the current player "killed" an "Other" player
   if sourceType == COMBAT_UNIT_TYPE_PLAYER and targetType == COMBAT_UNIT_TYPE_OTHER and (IsPlayerInAvAWorld() or IsActiveWorldBattleground())  then
      local s = "|C00CF34Killed " .. ZO_LinkHandler_CreatePlayerLink(target) .. "|r"   -- the print string message -> s
	 ...
	 if KC_G.savedVars.kbSpells[abilityName] == nil then
	    ...
	    KC_G.savedVars.kbSpells[abilityName] = KC_G.savedVars.kbSpells[abilityName] + 1
	 end
	 
	 if currentSession.kbSpells[abilityName] == nil then
	    currentSession.kbSpells[abilityName] = 1
	 else
	    currentSession.kbSpells[abilityName] = KC_G.savedVars.kbSpells[abilityName] + 1
	 end
	 ...
	 if not justKilled[target] or (time - justKilled[target]) > 2000 then -- if not just killed or killed more that 1000 milliseconds ago
	    ...
	    KC_G.savedVars.players[target][KillingBlows] = KC_G.savedVars.players[target][KillingBlows] + 1
	    currentSession.players[target][KillingBlows] = currentSession.players[target][KillingBlows] + 1
...
function KC_G.OnKilled(eventCode)
   ...
   if streak > 0  then 
	 ...
	 d(string.format("|C9C9C9CYour kill streak has ended! You had a streak of %d (%d KBStreak)|r", streak, kbStreak))
      end
   else
   ...
   if isPlayer then
	 ...
	 savedPlayer[Alliance], savedPlayer[Level], savedPlayer[vLevel] = alliance, level, vlevel
	 if reticle_data[killer] ~= nil then
	 ...
	 elseif savedPlayer ~= nil then
      ...
      KC_G.UpdateSessionData()
      d(string.format("|C870505You were killed by|r %s%s|r.", KC_Fn.Alliance_Color(alliance), ZO_LinkHandler_CreatePlayerLink(killer)))
      KC_G.UpdateDisplayText()
   end
...
function KC_G.doStreak(doKB)
   ...
   if streak_array[streak] ~= nil then
      if KC_Fn.CheckSetting("ChatKStreak") then
	 d(string.format("|CFFA203%s (%d Kills).|r", streak_array[streak], streak))
   ...
   if kb_streak_array[kbStreak] ~= nil and doKB then
      if KC_Fn.CheckSetting("ChatKStreak") then
	 d(string.format("|C878787%s (%d KBs).|r", kb_streak_array[kbStreak], kbStreak))
...
function KC_G.doDeathStreak()
   ...
   if deathStreak_array[deathStreak] ~= nil and KC_Fn.CheckSetting("ChatDStreak") then
      d(string.format("|C9C9C9C%s (%d Deaths)|r", deathStreak_array[deathStreak], deathStreak))
...
do
   ...
   local function CombineKillTables(killed, killedBy)
	 ...
	 if type(v[Alliance]) ~= "number" then v[Alliance] = AllianceStringToNumber(v[Alliance]) end
	 ...
	 if p then -- if already in list, grab some 
	    p[Level], p[vLevel], p[KilledBy], p[Alliance] = v.Level, v.vLevel, v.KilledBy, v.Alliance
   ...
   function KC_G.OnAddOnLoaded(eventCode, addOnName)
	 ...
	 KC_G.savedVars.players = CombineKillTables(KC_G.savedVars.killed, KC_G.savedVars.killedBy)
	 ...
	 for k, v in pairs(KC_G.savedVars.players) do
	    assert(v[Class])
	    KC_G.newClassData[v[Class]] = KC_G.newClassData[v[Class]] + v[Kills]
	 end
      end

      
      KillCounter:ClearAnchors()
      KillCounter:SetAnchor(KC_G.savedVars.anchorPoint, GuiRoot, nil, KC_G.savedVars.offsetX, KC_G.savedVars.offsetY)
...
function KC_G.OnCharacterLoad(eventCode, initialLoad)
   if IsPlayerInAvAWorld() or IsActiveWorldBattleground() then
	 ...
	 EVENT_MANAGER:RegisterForEvent(KC_G.name, EVENT_OBJECTIVE_CONTROL_STATE, SC_G.ObjectiveControlState)
      end
   elseif KC_ON then
      KC_G.show()
   else
...
function KC_G.QueueEntered(eventCode, campaignId, isGroup, position)
   KC_G.show(true)
   KillCounter_QueueStatus:SetText(string.format("Queue Status: %s (%d)",GetCampaignName(campaignId), position or GetCampaignQueuePosition(campaignId)))
...
function KC_G.QueueStateChange(eventCode, id, isGroup, state)
   ...
   if state == CAMPAIGN_QUEUE_REQUEST_STATE_CONFIRMING then
      KillCounter_QueueStatus:SetText(string.format("Queue Status: %s (Accept?)",GetCampaignName(campaignId)))
      ConfirmCampaignEntry(id, isGroup, true)
   elseif state == CAMPAIGN_QUEUE_REQUEST_STATE_PENDING_ACCEPT then
      KillCounter_QueueStatus:SetText(string.format("Queue Status: %s (Entering Cyrodiil...)",GetCampaignName(campaignId)))
...
function KC_G.QueueControl(activate)
   if activate then
      EVENT_MANAGER:RegisterForEvent(KC_G.name, EVENT_CAMPAIGN_QUEUE_JOINED, KC_G.QueueEntered)
      EVENT_MANAGER:RegisterForEvent(KC_G.name, EVENT_CAMPAIGN_QUEUE_LEFT, KC_G.QueueExited)
      EVENT_MANAGER:RegisterForEvent(KC_G.name, EVENT_CAMPAIGN_QUEUE_POSITION_CHANGED, KC_G.QueuePositionChange)
   else
      EVENT_MANAGER:UnregisterForEvent(KC_G.name, EVENT_CAMPAIGN_QUEUE_POSITION_CHANGED)
...
function KC_G.AutoQueueAccept(activate)
   if activate then
      EVENT_MANAGER:RegisterForEvent(KC_G.name, EVENT_CAMPAIGN_QUEUE_STATE_CHANGED, KC_G.QueueStateChange)
   else
      EVENT_MANAGER:UnregisterForEvent(KC_G.name, EVENT_CAMPAIGN_QUEUE_STATE_CHANGED)
   end
...
function KC_G.OnInitialized(self)
   ...
   EVENT_MANAGER:AddFilterForEvent(KC_G.name, EVENT_COMBAT_EVENT, REGISTER_FILTER_COMBAT_RESULT, ACTION_RESULT_KILLING_BLOW)
   ...
   EVENT_MANAGER:RegisterForEvent(KC_G.name, EVENT_ADD_ON_LOADED, KC_G.OnAddOnLoaded)
   EVENT_MANAGER:RegisterForEvent(KC_G.name, EVENT_ACTION_LAYER_PUSHED, KC_G.onActionLayerPushed)
   EVENT_MANAGER:RegisterForEvent(KC_G.name, EVENT_ACTION_LAYER_POPPED, KC_G.onActionLayerPopped)
   EVENT_MANAGER:RegisterForEvent(KC_G.name, EVENT_PLAYER_COMBAT_STATE, KC_G.CombatState)
   ...
   EVENT_MANAGER:RegisterForEvent(KC_G.name, EVENT_PLAYER_ACTIVATED, KC_G.OnCharacterLoad)
...
function  KC_G.AvengeKill(eventCode, avengedPlayer, killedPlayer)
   ...
   if KC_G.savedVars.players[n] == nil then
	 ...
	 t[Alliance] = reticle_data[n][Alliance]
      ...
      KC_G.savedVars.players[n][Avenge_Kills] = KC_G.savedVars.players[n][Avenge_Kills] + 1
   end
   KC_G.UpdateDisplayText()
end
...
function  KC_G.RevengeKill(eventCode, killedPlayer)
   ...
   if KC_G.savedVars.players[n] == nil then
	 ...
	 t[Alliance] = reticle_data[n][Alliance]
      ...
      KC_G.savedVars.players[n][Revent_Kills] = KC_G.savedVars.players[n][Revent_Kills] + 1
   end
   KC_G.UpdateDisplayText()
end
